...
class Perceptron(BaseEstimator):
    ...
    def update_coefs(self,X):
        original_X = X[:, :]
        if self.include_intercept_:
            X = np.c_[np.ones(X.shape[0]), X]  # Add a column of ones
            self.coefs_ = np.zeros(original_X.shape[1] + 1)
        else:
            self.coefs_ = np.zeros(original_X.shape[1])
        return X
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
            ...
            """
            X=self.update_coefs(X)
            for t in range(self.max_iter_):
                flag = False
                for i, x in enumerate(X):
                    if (np.dot(self.coefs_,x) * y[i]) <= 0:
                        self.coefs_ += x * y[i]
                        flag = True
    ...
    def _loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        return metrics.misclassification_error(y, self._predict(X))
